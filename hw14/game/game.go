...
func main() {
	...
	// Init observer.
	observer := NewObserverRegistrar()
	observer.Register(LoggerObserver{})
	...
	for i := 0; i < 5; i++ {
		...
		wg.Add(1)
		go playerRtn(ctx, p, guessChan, &wg, observer)
	}
...
func playerRtn(ctx context.Context, p *player, guesses chan guess, wg *sync.WaitGroup, observer *ObserverRegistrar) {
	defer wg.Done()
	...
	for {
		...
		case round := <-p.gameCh:
			...
			// Notify observers.
			observer.NotifyAll(fmt.Sprintf("player %d made guess for round %d.\n", p.id, round.id))
		}
	}
}
